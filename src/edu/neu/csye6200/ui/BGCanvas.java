...
public class BGCanvas extends JPanel implements Observer{
	...
	public void drawBG(Graphics g) {
		...
		if(BGApp.bgs.getBgSet().isEmpty() == false) {
			...
			for (int i = 0; i < BGApp.bgs.getBgSet().get(BGApp.BGSetCount).getBgs().size(); i++) {
				
				BGStem st = BGApp.bgs.getBgSet().get(BGApp.BGSetCount).getBgs().get(i); // get the current BGStem;
